...
const PriorityWithSliderSelection = ({ configuration, handleClose }) => {
  ...
  useEffect(() => {
    if (selected && selected.value) {
      ...
      if (selectedMandatories && selectedMandatories.length === values.length) {
        setMandatory(values)
        setSlider(selectedMandatories[0] ? selectedMandatories[0].valueNumber : 50)
      } else {
        setMandatory(selected.value.find(item => item.isOptional === false))
        setOptionals(selected.value.filter(item => item.isOptional === true))
        setSlider(selected.value[0] ? selected.value[0].valueNumber : 50)
      }
    }
  }, [selected, values])
  ...
  return (
    <Box display={'flex'} flexWrap={'wrap'} mt={4}>
      ...
      <Box mt={5} px={10} textAlign={'center'}>
        <Typography variant={'body2'} color={'textSecondary'}>
          {t('sliderText')}
        </Typography>
      </Box>
      <Box py={3} px={2} flexBasis={'100%'} display={'flex'} justifyContent={'center'} alignItems={'flex-end'}
        mx={'auto'}>
        <PriceSlider onChangeCommitted={(e, value) => setSlider(value)} value={slider} setValue={setSlider}/>
      </Box>
...
export default PriorityWithSliderSelection
