...
const FavoriteCard = ({ submission = {}, viewType }) => {
  ...
  return (
    <Box height={173}>
      <Box display='flex' justifyContent='space-between' pt={viewType === ViewType.TILE ? 1.8 : 1.5} pb={1}>
        ...
        {viewType === ViewType.ROW && <Box display='flex' justifyContent='space-between' width={50}>
      ...
      </Box>
      {
        submissionLocation.length > 0 &&
                <FormSectionLabel icon='locationFilled' iconSize={18} overflow color={theme.palette.text.secondary}
                  ...
                  <Box height={27} component={'div'} overflow='hidden'
                    textOverflow='ellipsis'>{submissionLocation}</Box>
                </FormSectionLabel>
      }
